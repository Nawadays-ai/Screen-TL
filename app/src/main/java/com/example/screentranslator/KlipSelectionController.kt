...
/**
 * Snip-like Manual TL mode. It intentionally reuses the already-running
 * FloatingService ScreenCaptureManager/TranslationManager instead of creating
 * a second MediaProjection session (Android 14+ requires one consent/session).
 *
 * The small reflection bridge is temporary architecture glue because the
 * existing service owns those private managers. A later refactor can move the
 * shared capture/translation session behind a public controller interface.
 */
object KlipSelectionController {
    ...
    fun start(context: Context, sourceView: View) {
        ...
        try {
            // Put the mask below the existing floating menu so the existing
            // Cancel button remains usable while Klip mode is active.
    ...
    private fun translateAll(
        ...
        index: Int,
        historyParts: MutableList<String>,
        overlays: MutableList<TranslationOverlayItem>,
        crop: Bitmap
    ) {
        if (!isActive) {
            currentBlurPatches(texts).forEach { patch -> runCatching { patch.recycle() } }
        ...
        if (index >= texts.size) {
            runCatching { crop.recycle() }
            if (overlays.isEmpty()) {
                fail("Tidak ada hasil terjemahan")
                return
            }
    ...
    private fun currentBlurPatches(texts: List<DetectedText>): List<Bitmap> = texts.mapNotNull { it.blurredPatch }
    ...
    private fun showOverlay(context: Context, items: List<TranslationOverlayItem>, width: Int, height: Int) {
        ...
        runCatching { wm.addView(view, params) }.onFailure {
            clipOverlayView = null
            Log.e(TAG, "Failed to show Klip overlay", it)
        }
    }
    ...
    private class KlipMaskView(
        ...
        private val selectionStrokePaint = Paint(Paint.ANTI_ALIAS_FLAG).apply {
            color = Color.WHITE; style = Paint.Style.STROKE; strokeWidth = SELECTION_STROKE
        }
        ...
        override fun onTouchEvent(event: MotionEvent): Boolean {
            when (event.actionMasked) {
                MotionEvent.ACTION_DOWN -> {
                    startX = event.x; startY = event.y; currentX = startX; currentY = startY; dragging = true; invalidate(); return true
                }
                MotionEvent.ACTION_MOVE -> {
                    currentX = event.x; currentY = event.y; invalidate(); return true
                }
                MotionEvent.ACTION_UP -> {
                    ...
                    dragging = false
                    invalidate()
                    val out = Rect(rect.left.toInt(), rect.top.toInt(), rect.right.toInt(), rect.bottom.toInt())
                    if (out.width() >= MIN_SELECTION_PX && out.height() >= MIN_SELECTION_PX) onSelectionComplete(out)
                    return true
                }
