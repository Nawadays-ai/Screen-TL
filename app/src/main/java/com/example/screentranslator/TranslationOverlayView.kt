...
class TranslationOverlayView(context: Context) : View(context) {
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        renderItems.forEachIndexed { index, renderItem ->
            ...
            val padding = renderItem.horizontalPadding * scaleX; val lineHeight = renderItem.lineSpacing * scaleY; val totalTextHeight = lineHeight * renderItem.lines.size
            val startTop = top + ((bottom - top - totalTextHeight) / 2f).coerceAtLeast(0f); val metrics = textPaint.fontMetrics; val firstBaseline = startTop - metrics.ascent
    ...
    private fun buildOverlapGroups() {
        ...
        itemGroups = groups.toList(); val sums = Array(rootToGroup.size) { FloatArray(4) }
        renderItems.forEachIndexed { index, item -> val group = itemGroups[index]; val color = darkenColor(item.item.backgroundColor); sums[group][0] = sums[group][0] + Color.red(color); sums[group][1] = sums[group][1] + Color.green(color); sums[group][2] = sums[group][2] + Color.blue(color); sums[group][3] = sums[group][3] + 1f }
    ...
    private fun drawReconstructedBlur(canvas: Canvas, box: RectF, baseColor: Int, radius: Float) {
        val edge = adjustColor(baseColor, 0.94f); val deep = adjustColor(baseColor, 0.88f); backgroundPaint.alpha = 255
    ...
    private fun buildRenderItem(item: TranslationOverlayItem, width: Int, height: Int): RenderItem? {
        ...
        if (totalHeight > availableHeight && totalHeight > 0f) {
            val fitScale = (availableHeight / totalHeight).coerceAtLeast(minFontScale); finalTextSize = (finalTextSize * fitScale).coerceIn(minTextSizePx, baseTextSize); lines = wrapText(normalized, maxTextWidth, finalTextSize)
        }
    ...
    private fun wrapText(text: String, maxWidth: Float, textSize: Float): List<String> {
        textPaint.textSize = textSize; val result = mutableListOf<String>()
