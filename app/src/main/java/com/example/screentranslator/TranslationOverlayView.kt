...
class TranslationOverlayView(context: Context) : View(context) {
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        // PASS 1: opaque reconstructed replacement backgrounds.
        ...
        // PASS 2: text is fully opaque and always drawn after backgrounds.
        renderItems.forEachIndexed { index, renderItem ->
            val left = renderItem.left * scaleX
            val top = renderItem.top * scaleY
            val right = renderItem.right * scaleX
    ...
    private fun buildOverlapGroups() {
        ...
        renderItems.indices.forEach { index ->
            val root = find(index); groups[index] = rootToGroup.getOrPut(root) { rootToGroup.size }
        }
    ...
    private fun darkenColor(color: Int): Int {
        ...
        // Previous visual level was 0.82. This is about 25% stronger visually.
    ...
    private fun adjustColor(color: Int, factor: Float): Int = Color.rgb(
    ...
    private fun buildRenderItem(item: TranslationOverlayItem, width: Int, height: Int): RenderItem? {
        ...
        val baseTextSize = (if (item.sourceTextSizePx > 0f) item.sourceTextSizePx else boxHeight * 0.72f).coerceIn(minTextSizePx, maxTextSizePx)
        val normalized = normalizeParagraph(item.translatedText)
        ...
        val maxBoxWidth = minOf(baseWidth * maxWidthRatio, (width - baseLeft - 4f).coerceAtLeast(baseWidth))
        ...
        val finalLineSpacing = finalTextSize * 1.16f
        lines = wrapText(normalized, maxTextWidth, finalTextSize)
        val finalMeasuredWidth = lines.maxOfOrNull { textPaint.measureText(it) } ?: 0f
        val fillWidth = (finalMeasuredWidth + horizontalPadding * 2f).coerceAtLeast(baseWidth).coerceAtMost(right - left)

        return RenderItem(item, left, baseTop, right, baseBottom, left + fillWidth, finalTextSize, horizontalPadding, lines, finalLineSpacing)
    }

    private fun normalizeParagraph(text: String): String = text
        .replace("\r\n", "\n")
        .replace('\r', '\n')
        .split('\n')
    ...
    private fun wrapText(text: String, maxWidth: Float, textSize: Float): List<String> {
        ...
        for (paragraphLine in text.split('\n')) {
            ...
            for (word in paragraphLine.split(' ')) {
                ...
                if (current.isEmpty() || textPaint.measureText(candidate) <= maxWidth) {
                    current = candidate
                } else {
                    result.add(current)
                    current = word
