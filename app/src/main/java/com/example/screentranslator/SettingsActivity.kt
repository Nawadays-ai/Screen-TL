...
class SettingsActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btnGeminiCheck = findViewById(R.id.btnGeminiCheck)
        btnGeminiUse = findViewById(R.id.btnGeminiUse)
        btnGeminiDisable = findViewById(R.id.btnGeminiDisable)
        etDeepLKey = findViewById(R.id.etDeepLKey)
        tvDeepLStatus = findViewById(R.id.tvDeepLStatus)
        btnDeepLCheck = findViewById(R.id.btnDeepLCheck)
        btnDeepLUse = findViewById(R.id.btnDeepLUse)
        btnDeepLDisable = findViewById(R.id.btnDeepLDisable)
        ...
        spinnerManualProvider.setOnItemSelectedListener(SimpleItemSelectedListener { position ->
            ApiSettings.setManualProvider(providers[position])
        })
    ...
    private fun checkGemini() {
        ...
        GeminiTranslationProvider(key, "Inggris", "Indonesia").prepare(
            onReady = runOnUiThread {
                ...
                renderGemini(successMessage = "API dapat digunakan")
            },
            onFailure = { error -> runOnUiThread {
                ApiSettings.setGeminiVerified(false)
                ApiSettings.setGeminiEnabled(false)
                tvGeminiStatus.text = "API tidak dapat digunakan${error.message?.let { ": $it" } ?: ""}"
                renderGeminiButtonsOnly()
            } }
        )
    }
    ...
    private fun checkDeepL() {
        ...
        DeepLTranslationProvider(key, "Inggris", "Indonesia").prepare(
            onReady = runOnUiThread {
                ...
                renderDeepL(successMessage = "API dapat digunakan")
            },
            onFailure = { error -> runOnUiThread {
                ApiSettings.setDeepLVerified(false)
                ApiSettings.setDeepLEnabled(false)
                tvDeepLStatus.text = "API tidak dapat digunakan${error.message?.let { ": $it" } ?: ""}"
                renderDeepLButtonsOnly()
            } }
        )
    }
    ...
    private fun renderGemini(successMessage: String? = null) {
        ...
        if (key.isBlank()) {
            tvGeminiStatus.text = ""
            btnGeminiCheck.visibility = Button.GONE
            btnGeminiUse.visibility = Button.GONE
            btnGeminiDisable.visibility = Button.GONE
            return
        }
        if (successMessage != null) tvGeminiStatus.text = successMessage
        else if (ApiSettings.isGeminiEnabled()) tvGeminiStatus.text = "API aktif"
        else if (ApiSettings.isGeminiVerified()) tvGeminiStatus.text = "API dapat digunakan"
        else tvGeminiStatus.text = "Belum dicek"
        btnGeminiCheck.visibility = Button.VISIBLE
        btnGeminiUse.visibility = if (ApiSettings.isGeminiVerified() && !ApiSettings.isGeminiEnabled()) Button.VISIBLE else Button.GONE
        btnGeminiDisable.visibility = if (ApiSettings.isGeminiEnabled()) Button.VISIBLE else Button.GONE
    }

    private fun renderGeminiButtonsOnly() {
        btnGeminiCheck.visibility = Button.VISIBLE
        btnGeminiUse.visibility = Button.GONE
        btnGeminiDisable.visibility = Button.GONE
    }
    ...
    private fun renderDeepL(successMessage: String? = null) {
        ...
        if (key.isBlank()) {
            tvDeepLStatus.text = ""
            btnDeepLCheck.visibility = Button.GONE
            btnDeepLUse.visibility = Button.GONE
            btnDeepLDisable.visibility = Button.GONE
            return
        }
        if (successMessage != null) tvDeepLStatus.text = successMessage
        else if (ApiSettings.isDeepLEnabled()) tvDeepLStatus.text = "API aktif"
        else if (ApiSettings.isDeepLVerified()) tvDeepLStatus.text = "API dapat digunakan"
        else tvDeepLStatus.text = "Belum dicek"
        btnDeepLCheck.visibility = Button.VISIBLE
        btnDeepLUse.visibility = if (ApiSettings.isDeepLVerified() && !ApiSettings.isDeepLEnabled()) Button.VISIBLE else Button.GONE
        btnDeepLDisable.visibility = if (ApiSettings.isDeepLEnabled()) Button.VISIBLE else Button.GONE
    }

    private fun renderDeepLButtonsOnly() {
        btnDeepLCheck.visibility = Button.VISIBLE
        btnDeepLUse.visibility = Button.GONE
        btnDeepLDisable.visibility = Button.GONE
    }
