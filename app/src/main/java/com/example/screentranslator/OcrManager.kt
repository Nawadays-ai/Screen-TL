...
class OcrManager(sourceLanguage: String) {
    ...
    private val mangaMode: Boolean
        get() = TranslationModeSettings.isMangaMode()
    ...
    private var mangaOcrManager: MangaOcrManager? = null
    ...
    fun recognize(bitmap: Bitmap, onSuccess: (List<DetectedText>) -> Unit, onFailure: (Exception) -> Unit, trace: ScreenTLPerformanceTrace? = null) {
        if (mangaMode) {
            if (mangaOcrManager == null) mangaOcrManager = MangaOcrManager(TranslationModeSettings.context())
            mangaOcrManager!!.recognize(bitmap, onSuccess, onFailure, trace)
            return
        }
        recognizeNormal(bitmap, onSuccess, onFailure, trace)
    }

    private fun recognizeNormal(bitmap: Bitmap, onSuccess: (List<DetectedText>) -> Unit, onFailure: (Exception) -> Unit, trace: ScreenTLPerformanceTrace?) {
        val perfTrace = trace ?: ScreenTLPerformanceTrace.current()
        perfTrace?.mark("ocr_start engine=mlkit")
        ...
        recognizer.process(image).addOnSuccessListener { visionText ->
            ...
            for (block in visionText.textBlocks) {
                val blockText = TextPreprocessor.normalize(block.text.trim(), mangaMode = false)
                ...
                if (TextLayoutAnalyzer.isVerticalBlock(block)) {
                    ...
                    if (layout != null) detectedTexts.add(DetectedText(TextPreprocessor.normalize(TextLayoutAnalyzer.verticalBlockText(block), false), layout.left, layout.top, layout.right, layout.bottom, layout.sourceTextSizePx, layout.backgroundColor, layout.blurredPatch, TextLayoutAnalyzer.WritingOrientation.VERTICAL))
                    continue
                }
                if (TextLayoutAnalyzer.shouldTreatAsParagraph(block)) {
                    ...
                    for (line in block.lines) {
                        val lineText = TextPreprocessor.normalize(line.text.trim(), false)
            ...
            for (group in verticalGroups) {
                ...
                val text = TextPreprocessor.normalize(group.sortedByDescending { it.box.centerX() }.joinToString("") { it.text }, false)
            ...
            val ordered = SmartTextLayout.sort(detectedTexts, mangaMode = false)
            perfTrace?.mark("mlkit_result blocks=${visionText.textBlocks.size} lines=${visionText.textBlocks.sumOf { it.lines.size }}")
            perfTrace?.mark("ocr_geometry_complete paragraphs=$paragraphCount lines=$lineCount verticalGroups=${verticalGroups.size} detected=${ordered.size}")
            ordered.forEachIndexed { index, detected -> Log.d(TAG, "[$index] ${detected.orientation} chars=${detected.text.length} box=${detected.left},${detected.top},${detected.right},${detected.bottom}") }
    ...
    private fun mergeVerticalLayouts(group: List<VerticalCandidate>): TextLayoutAnalyzer.Result? {
        ...
        return TextLayoutAnalyzer.Result(
            group.minOf { it.layout.left },
            group.minOf { it.layout.top },
            group.maxOf { it.layout.right },
            group.maxOf { it.layout.bottom },
            group.map { it.layout.sourceTextSizePx }.average().toFloat(),
            group.first().layout.backgroundColor,
            TextLayoutAnalyzer.WritingOrientation.VERTICAL
        )
    }

    fun close() {
        mangaOcrManager?.close()
        mangaOcrManager = null
        recognizer.close()
    }
}
