...
object TextLayoutAnalyzer {
    ...
    /**
     * Samples the dominant background colour of the box the text sits in.
     *
     * Earlier revisions sampled thin strips just outside the box, which returned the colour of
     * whatever surrounded the control (scene, grass, a neighbouring panel) rather than the control
     * itself. Sampling the interior instead is what lets the Manual overlay take on the button's
     * own colour. The text itself is mostly transparent between glyphs at this sampling density,
     * so the median of the interior is the button's fill, and the median is used rather than the
     * mean so a stray bright glyph cannot skew it.
     */
    private fun estimate(bitmap:Bitmap,left:Int,top:Int,right:Int,bottom:Int):Int{val s=ArrayList<Int>(128);val w=right-left;val h=bottom-top;if(w<=0||h<=0)return Color.BLACK;val sx=(w/16).coerceAtLeast(1);val sy=(h/12).coerceAtLeast(1);for(x in left until right step sx)s.add(bitmap.getPixel(x.coerceIn(0,bitmap.width-1),(top+h/2).coerceIn(0,bitmap.height-1)));for(y in top until bottom step sy)s.add(bitmap.getPixel((left+w/2).coerceIn(0,bitmap.width-1),y.coerceIn(0,bitmap.height-1)));if(s.isEmpty())return Color.BLACK;val rs=s.map(Color::red).sorted();val gs=s.map(Color::green).sorted();val bs=s.map(Color::blue).sorted();val m=s.size/2;return Color.rgb(rs[m],gs[m],bs[m])}
}
